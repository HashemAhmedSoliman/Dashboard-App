...
import { getPeriodLabel } from '../utils/periodLabels';
...
export default function SalesPopup({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    if (cache.has(selectedPeriod)) {
      setData(cache.get(selectedPeriod));
      buildTrend(cache.get(selectedPeriod));
      return;
    }
    setLoading(true);
  ...
  const buildTrend = (d: any) => {
    const rows = d?.trend ?? [];
    setTrendData(rows.map((r: any) => ({ value: n(r?.netsales) })));
  };
  ...
  const kpis: KpiItem[] = [
    ...
    { label: t('MD_InvoicesCount'),  value: n(s?.netinvoicecount).toLocaleString() },
    { label: t('MD_AvgInvoice'),     value: fmt(s?.avginvoice) },
  ...
  const salespeopleCols: Column[] = [
    { key: 'salespersonname', label: t('MD_Tbl_Salesperson'), flex: 2 },
    { key: 'netsales',        label: t('MD_Tbl_Revenue'),     flex: 1, numeric: true, render: (v) => fmt(v) },
  ];
  const itemCols: Column[] = [
    { key: 'itemname', label: t('MD_Tbl_Item'),     flex: 2 },
    { key: 'quantity', label: t('MD_Tbl_Qty'),      flex: 1, numeric: true },
    { key: 'netsales', label: t('MD_Tbl_Revenue'),  flex: 1, numeric: true, render: (v) => fmt(v) },
  ];
  const taxCols: Column[] = [
    { key: 'description', label: t('MD_Tbl_Type'),   flex: 2 },
    { key: 'amount',      label: t('MD_Tbl_Amount'), flex: 1, numeric: true, render: (v) => fmt(v) },
  ];
  const paymentCols: Column[] = [
    { key: 'paymenttype', label: t('MD_Tbl_Type'),   flex: 2 },
    { key: 'amount',      label: t('MD_Tbl_Amount'), flex: 1, numeric: true, render: (v) => fmt(v) },
  ];
    ...
    >
      ...
      {trendData.length > 0 && (
        <SectionCard title={t('MD_Pop_HistoricalTrend')}>
          <SparkLine data={trendData} color={ACCENT} height={160} />
        </SectionCard>
      )}
